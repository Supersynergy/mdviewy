...
```bash
git clone https://git.marketdeck.io/Supersynergy/mdviewy.git
cd mdviewy
bun install
```
...
Full verification path: `bun run test && bun run lint && just fmt && just check && bun run dev && bun run build && bun install`
