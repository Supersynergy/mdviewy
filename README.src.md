...
- **Reliable native open:** Finder, CLI and warm-instance opens preserve real paths and switch single files into a distraction-free focus layout.<!--en-->
- **Adaptive editor:** WYSIWYG and source modes, multiple tabs, typewriter scrolling, spellcheck, and adaptive/focused/wide/full document widths.<!--en-->
- **Fast navigation:** Command palette, recent tabs, file search, native Rust full-text search, Table of Contents, and reveal-in-folder actions.<!--en-->
- **Rich Markdown:** Math plus 19 bundled Mermaid diagram types, wide-table handling, page breaks, and responsive local/remote media.<!--en-->
- **Smart Actions:** Detect local paths and GitHub repositories, copy agent-ready context/handoffs, and create deterministic document briefs.<!--en-->
- **Frontmatter insights:** Titles and tags appear in Smart Actions without changing the document.<!--en-->
- **Built-in AI:** OpenAI-compatible, DeepSeek, Google and local Ollama providers with explicit opt-in settings.<!--en-->
- **Export:** HTML, image, and Print / Save as PDF with print-only document styling.<!--en-->
- **Workspace tools:** File tree, drag-and-drop, bookmarks, image asset rules, custom themes, and keyboard shortcuts.<!--en-->
- **Cross-platform and multilingual:** macOS, Windows and Linux; Chinese, English, Spanish, Japanese and French.<!--en-->
- **可靠的原生打开**：Finder、命令行和已运行实例都能保留真实路径；单文件打开会进入无干扰专注布局。<!--zh-->
- **自适应编辑器**：所见即所得与源码模式、多标签、打字机滚动、拼写检查，以及自适应/专注/宽/全宽文档布局。<!--zh-->
- **快速导航**：命令面板、最近标签、文件搜索、Rust 原生全文搜索、目录和在文件夹中显示。<!--zh-->
- **丰富 Markdown**：数学公式、19 种内置 Mermaid 图表、宽表格、分页符和响应式媒体。<!--zh-->
- **智能操作**：识别本地路径与 GitHub 仓库，复制适合智能体的上下文/交接提示，并生成确定性的文档摘要。<!--zh-->
- **Frontmatter 洞察**：在不修改文档的情况下显示标题和标签。<!--zh-->
- **内置 AI**：支持 OpenAI 兼容、DeepSeek、Google 和本地 Ollama，且需在设置中明确启用。<!--zh-->
- **导出**：HTML、图片，以及带专用打印样式的“打印 / 保存为 PDF”。<!--zh-->
- **工作区工具**：文件树、拖放、书签、图片资源规则、自定义主题和快捷键。<!--zh-->
- **跨平台与多语言**：macOS、Windows、Linux；中文、英文、西班牙语、日语和法语。<!--zh-->
- **確実なネイティブオープン:** Finder、CLI、起動済みインスタンスから正しいパスで開き、単一ファイルは集中レイアウトに切り替わります。<!--ja-->
- **適応型エディター:** WYSIWYG/ソース、複数タブ、タイプライタースクロール、スペルチェック、適応/集中/ワイド/全幅。<!--ja-->
- **高速ナビゲーション:** コマンドパレット、最近のタブ、ファイル検索、Rust全文検索、目次、フォルダー表示。<!--ja-->
- **豊富なMarkdown:** 数式、19種類のMermaid図、幅広テーブル、改ページ、レスポンシブなメディア。<!--ja-->
- **Smart Actions:** ローカルパスとGitHubリポジトリを検出し、エージェント向け文脈/引き継ぎと文書ブリーフを生成します。<!--ja-->
- **Frontmatter情報:** 文書を変更せず、タイトルとタグをSmart Actionsに表示します。<!--ja-->
- **内蔵AI:** OpenAI互換、DeepSeek、Google、ローカルOllamaを明示的な設定で利用できます。<!--ja-->
- **エクスポート:** HTML、画像、文書専用スタイル付きの「印刷 / PDFとして保存」。<!--ja-->
- **ワークスペース機能:** ファイルツリー、ドラッグ&ドロップ、ブックマーク、画像ルール、テーマ、ショートカット。<!--ja-->
- **クロスプラットフォーム/多言語:** macOS、Windows、Linuxと、中国語、英語、スペイン語、日本語、フランス語。<!--ja-->
