...
- **超轻量**：mdviewy 基于 tauri, 拥有小于 20MB 的体积和更好的性能。
- **多编辑模式**：mdviewy 使用 prosemirror 作为编辑器核心, 不仅扩展性高，编辑体验也很好。并支持多种编辑模式，如`source code`, `wysiwyg`。
- **编辑多种文件**：除了 Markdown，还支持编辑 `json`、`txt` 等文件类型。
- **自定义主题**：支持自定义主题，并且你也可以与他人分享你的主题。
- **自定义快捷键**：支持自定义快捷键，满足个性化需求。
- **图片处理**：当你粘贴图片到 mdviewy，可以选择粘贴到指定的路径，或者转为 `base64`。
