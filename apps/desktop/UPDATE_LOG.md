...
I wrote an article on how to use Copilot and Ollama in mdmaster. If you encounter some problems, you may find solutions here, [Use Copilot with Ollama](https://www.mdmaster.cc/en/docs/Extension/UseCopilotWithOllama)。
...
我写了一篇如何在 mdmaster 中使用 Copilot 和 Ollama，如果你遇到了一些问题，也许可以在这里找到解决方案, [如何Copilot 与 Ollama 配合使用](https://www.mdmaster.cc/zh/docs/Extension/UseCopilotWithOllama)。
...
- Support for multiple system right-clicks to quickly open files with `mdmaster`.
...
- `WYSIWYG` mode supports drag-and-drop image upload, defaulting to storage in the application's `${mdmasterConfigPath}/assets/images` directory. The completed path can be viewed and modified in `Settings - Pictures`.
...
- 所见即所得模式支持图片的拖拽上传，默认将存放在应用的 `${mdmasterConfigPath}/assets/images` 目录。完成路径可以在 `设置 - 图片`中查看和修改。
